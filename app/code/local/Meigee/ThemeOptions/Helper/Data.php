 ...
 public function getThemeOptions ($themeOption) {
 	switch ($themeOption) {
		...
		break;
 		...
 		break;
 		case 'menu':
 			return Mage::getStoreConfig('meigee_general/menu/type');
 		break;
 		case 'sociallinks':
 		...
 		break;
 		case 'homeButton':
 			return Mage::getStoreConfig('meigee_general/menu/home');
 		break;
 		case 'labelnew':
	 	...
	 	if ($imgcount>0):
		 	...
		 	foreach ($_gallery as $_image ):
		        if ($_image->getLabel() == 'hover'):
		        	echo '<span class="hover-image"><img src=' . Mage::helper('catalog/image') -> init($_product, 'thumbnail', $_image -> getFile())->constrainOnly(TRUE)->keepAspectRatio(TRUE)->keepFrame(FALSE) -> resize($size, null) . ' alt="" /></span>';
		 		break;
		    	endif;
	        endforeach;
		endif;
 ...
 public function prevnext ($product) {
 	if ($this->getThemeOptions('productpage_prevnext') == 'prevnext'):
		...
		*/
		...
		if ($url <> $next_url): 
			$html = '<a class="product-prev" href="' . $next_url . '">PREV</a>'; 
	    endif;
	    if ($url <> $prev_url):
	        $html .= '<a class="product-next" href="' . $prev_url . '">NEXT</a>';
	    endif;
